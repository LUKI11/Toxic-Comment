# -*- coding: utf-8 -*-
"""
Created on Wed May  2 09:47:41 2018
...
def get_model():
    ...
    embed_size = 128
    # embed a list of the coordinates of the words in this vector space
    x = Embedding(len(tokenizer.word_index)+1, embed_size)(inp)
    #use CNN
    x = Conv1D(filters=100,kernel_size=4,padding='same', activation='relu')(x)
    #pass it to the max pooling layer that applies the max pool operation on a window of every 4 characters(still 3D matrix)
...
submission.to_csv('../result/submission_CNN.csv', index=False)
